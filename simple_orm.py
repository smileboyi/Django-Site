...
def create_pool(loop, **kw): #这里的**kw是一个dict  
    ...
    #这里有一个关于Pool的连接，讲了一些Pool的知识点，http://aiomysql.readthedocs.io/en/latest/pool.html，以及destroy__pool里面的wait_closed() 
...
if __name__=="__main__":#一个类自带前后都有双下划线的方法，在子类继承该类的时候，这些方法会自动调用，比如__init__  
    class Runoob_tbl(Model): #虽然User类乍看没有参数传入，但实际上，User类继承Model类，Model类又继承dict类，所以User类的实例可以传入关键字参数  
        runoob_id = IntegerField('runoob_id',primary_key=True) #主键为id， tablename为User，即类名  
        runoob_title = StringField('runoob_title')  
        runoob_author = StringField('runoob_author')  
    ...
    @asyncio.coroutine  
    def test():  
        yield from create_pool(loop=loop, host='localhost', port=3306, user='root', password='', db='runoob')  
        r = yield from Runoob_tbl.find("5")  
        print(r)  

        # Runoob = Runoob_tbl(runoob_id=21, runoob_title='Tom学习3', runoob_author='Tom', submission_date='2017-11-15')  
        # yield from Runoob.save()
        # #yield from Runoob.update() 
        # yield from Runoob.delete()
        # print(Runoob) 
